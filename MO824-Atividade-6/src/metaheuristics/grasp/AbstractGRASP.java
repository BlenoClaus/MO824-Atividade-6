...
public abstract class AbstractGRASP<E> {
	...
	/**
	 * Creates the Candidate List, which is an List of candidate elements
	...
	/**
	 * Creates the Restricted Candidate List, which is an List of the best
	...
	public Solution<E> solve() {
		...
		for (i = 0; i < iterations && totalTempo < thirtyMinutes; i++) {
			...
			if (bestSol.cost > incumbentSol.cost) {
				...
				if (verbose)
					Log.info("\t(Iter. " + i + ") BestSol = " + bestSol);
					Log.info("\tTempo = " + (double)(System.currentTimeMillis() - startTime)/(double)1000);
			}
